...
var ykp = {
    ...
    //api_url: 'http://api.fin.me',
   // api_url: 'http://api-oa.smnf.cc', //发布时用这个地址
...
//          it.showBlock();
        ...
        var defaultOptions = {
            ...
            success: function (res) {
            	...
            	$("thead input[type='checkbox'],tbody input[name='check']").each(function(i, n) {
					n.checked = false;
				});
...
//      option.data.request_url = it.api_url + ajaxOption.url;
    ...
    list: function (obj) {
        ...
        postData.order = postData.order ? postData.order : (ykp.getCookie("order") ? ykp.getCookie("order") : "");
        var option = $.extend({}, obj);
        var div = document.createElement("div");
        var html = '<div class="dataTables_processing" id="onProcess" style="">' + '<div class="note-success" style="padding:10px;border-radius:10px;">' + '<img  style="margin-right:10px;top:-1px;display: inline-block; position:relative" src="data:image/png;base64," class="rotate"/>' + '<span style="font-size:14px;">' + '数据玩命加载中...' + '</div>' + '</div>'
        div.innerHTML = html;
...
//          $('body').append(div);
        ...
        it.doAjax({
            ...
            success: function (res) {
                ...
                var totalCount = res.data.totalCount || 0;
                var contId = (option.datacont && option.datacont.id) ? option.datacont.id :"#num";;
                if($(".modal-backdrop").length != 0){
                	$('#templateBox #num').html('共' +  totalCount  + '条数据');
                	$('#templateBox ._num').html('共' + totalCount + '条数据');
              	$(contId).html('共' +  totalCount  + '条数据');
              	$('._num').html('共' + totalCount + '条数据');
                }else{
                    $(contId).html('共' +  totalCount  + '条数据');
                	$('._num').html('共' + totalCount + '条数据');
                }
                it.setCookie('filterData',JSON.stringify(obj.data));
                if (option.loadList && typeof option.loadList === 'function') {
                    ...
                    if (page_no && total && option.pageBar && option.pageBar.id) { //分页组件
                        ...
                        it.setCookie('page_no',page_no);
                        if(typeof obj.data.page_size == 'undefined'){
                        	p_size = 10;
                        }else {
                        	p_size = obj.data.page_size;
                        }
                        //it.setCookie('page_size',p_size);
	...
	showBlock2:function(options){
		...
		var loading_width = "style='width:"+options.width+"px !important'";
		$('.loading-box-bg').remove();
		var html = [
		'<div class="loading-box-bg"><div class="loading-box" '+loading_width+'><div class="spinner"><div></div>',
		'<div></div><div></div><div></div><div></div><div></div></div><div class="circlebg"></div>',
		'<div class="loading-text">'+options.str+'</div>',
		'</div></div>',		
		];				
        $("body").prepend(html.join(''));
	},
	hideBlock2:function(time,callback){
		var time = time || 0;
        setTimeout(function () {
            $('.loading-box-bg').remove();
        }, time); 
...
//            '</div>',
            ...
            body_width, 'px;text-align:center;z-index:9999;position:fixed;border:3px solid #DEDEDE;left:45%;top:45%;">',
            '<div class="msg sp_left" style="padding: 20px 15px;word-wrap:break-word;">',
            msg || '',
            '</div>',
            '<div class="tools" style="border-top:1px solid #eee;">',
            '</div>',
            '</div>',
            '</div>'];
        $('body').prepend(html.join(""));
        ...
        for (var i in buttons) {
            var item = buttons[i];
            var buttonHtml = ['<div class="sp_left" style="line-height:45px;cursor:pointer;width:50%;display:inline-block;" id="tools_button',
    ...
    prompt: function (str, option) {
        ...
        var html = [
            '<div class="' + id + '" class="global_shade" style="z-index:10000000000;position:fixed;left:0;right:0;top:0;bottom:0;">',
            '<div style="width:260px;height: 45px;position: fixed;left:50%;top:50%;margin-left:-130px; z-index:1050">',
            '<p style="color:#fff;width:100%;line-height: 35px;margin: 0 auto;padding: 0px 15px;text-align:center;font-size:14px;box-shadow: 0 2px 3px #9d9d9d;background:#484848;border-radius:5px;">',
            str.indexOf("ID") != -1 ? str.replace(/ID/g,"") : str,
            '</p>',
            '</div>',
            '</div>,'
        ];
